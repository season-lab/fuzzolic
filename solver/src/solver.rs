...
use z3::{Context, ast::{Ast, Bool, BV}};
...
impl SMTSolver {
    ...
    fn build_memchr_bool<'a>(&'a self, s: &BV<'a>, needle: u8, n: usize) -> Bool<'a> {
        ...
        if n == 0 {
            return Z3Bool::from_bool(&self.ctx, false);
        }
        ...
        for i in 0..n {
            ...
            acc = Some(match acc {
                None => eq,
                Some(prev) => Z3Bool::or(&self.ctx, &[&prev, &eq]),
            });
        }
        acc.unwrap()
    }
...
pub struct SMTSolver {
    ...
    // Cache of constraints indexed by input id and query idx, similar in spirit to z3_ast_exprs[] and update_and_add_deps_to_solver()
    constraints_by_input: std::cell::RefCell<std::collections::HashMap<usize, Vec<ConstraintRecord>>>,
    constraints_by_query: std::cell::RefCell<std::collections::HashMap<usize, ConstraintRecord>>,
}
...
impl SMTSolver {
    pub fn new(config: &crate::Config) -> Result<Self> {
        ...
        Ok(SMTSolver {
            ...
            constraints_by_input: std::cell::RefCell::new(std::collections::HashMap::new()),
            constraints_by_query: std::cell::RefCell::new(std::collections::HashMap::new()),
        })
    }
    ...
    fn build_stride_cmpeq_bool<'a>(&'a self, s1: &BV<'a>, s2: &BV<'a>, len: usize) -> Bool<'a> {
        use z3::ast::Bool as Z3Bool;
        if len == 0 {
            return Z3Bool::from_bool(&self.ctx, true);
        }
        ...
        while start < total_bits {
            ...
            acc = Some(match acc {
                None => eq,
                Some(prev) => Z3Bool::and(&self.ctx, &[&prev, &eq]),
            });
            start = end;
        }
        acc.unwrap()
    ...
    fn build_strlen_bool<'a>(&'a self, s: &BV<'a>, s1_len: usize, n: usize) -> Bool<'a> {
        ...
        for i in 0..s1_len {
            ...
            acc = Some(match acc {
                None => neq_zero,
                Some(prev) => Z3Bool::and(&self.ctx, &[&prev, &neq_zero]),
            });
        }
        ...
        if n == 0 || s1_len < n {
            ...
            acc = Some(match acc {
                None => eq_zero,
                Some(prev) => Z3Bool::and(&self.ctx, &[&prev, &eq_zero]),
            });
        }
        match acc {
            Some(b) => b,
            None => Z3Bool::from_bool(&self.ctx, true),
        }
    }
            ...
            OpKind::Bswap => {
                ...
                if let Some(op_bv) = operand.as_bv() {
                    ...
                    for i in 0..bytes {
                        let hi = (i + 1) * 8 - 1;
                        let lo = i * 8;
                        let byte = op_bv.extract(hi as u32, lo as u32);
