 ...
 * `--afl AFL_WORKDIR`: this enables the AFL model, ee next section;
 * `--timoeut TIMEOUT`: maximum cumulative solving time (ms) within one path exploration;
 * `--optimistic-solving`: this enables optimistic solving;
 * `--single-path`: fuzzolic will perform a single-path exploration (first input fron the input directory)
 * `--keep-run-dirs`: intermediate run directories (`workdir/fuzzolic-XXXXX`), containing tracer/solver logs and generated testcases (before discarding uninteresting ones), will not be deleted;
 * `--address-reasoning`: fuzzolic will try to generate testcases that cover different memory addresses when finding a symbolic pointer.
...
* `fuzzolic-XXXXX/` (kept only when --keep-run-dirs` is used): e.g., `{fuzzolic-00000, fuzzolic-00001, ...}`
	...
	* `test_case_ZZZ_YYY.datg`: e.g., `test_case_0_0.dat`, one test case generated by fuzzolic for path id=XXXXXX when processing query with id=ZZZ
* `queue/`: 
	* when in standalone mode: test cases (from the input directory or from previous fuzzolic-XXXXX/ dictories) that still needs to be processed by fuzzolic
...
Hence, when looking for interesting test cases generated by fuzzolic, check the directory `tests` when using fuzzolic in standalone mode, or check the directory `queue` when using fuzzolic in AFL mode.
